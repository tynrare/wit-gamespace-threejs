import * as THREE from "three";
import {
  AnimationNode,
  Animator,
  AnimationMachine,
  ANIMATION_PLAYBACK_MODE,
  ANIMATION_TRANSITION_MODE,
} from "../animator";
import { Vec3Up, Vec3Forward, angle_sub, cache } from "../math.js";
import { oimo } from "../lib/OimoPhysics.js";

const oimo_vec3_0 = new oimo.common.Vec3();
const oimo_quat_0 = new oimo.common.Quat();

class PawnDrawA {
  constructor() {
    /** @type {THREE.Object3D} */
    this._target = null;
    /** @type {Animator} */
    this.animator = null;

    this.pos = new THREE.Vector3();
    this.velocity = new THREE.Vector3();
    this.goal = new THREE.Vector3();

    /** @type {oimo.dynamics.rigidbody.RigidBody} */
    this.body = null;

    this.allow_move = true;
  }

  /**
   * @param {THREE.Object3D} target
   */
  init(gltf, target) {
    this.animator = new Animator();
    this._target = target;
    this.gltf = gltf;
    this.pos.copy(this._target.position);
    this.goal.copy(this._target.position);

    this.animator.init(this._target, this.gltf);

    const am = this.animator.animation_machine;
    const register = (
      name,
      clipname,
      { playback_mode = ANIMATION_PLAYBACK_MODE.default, speed = 1 } = {},
    ) => {
      const clip = this.animator.getAnimation(clipname);
      if (!clip) {
        throw new Error(
          `Animator::register error - no clip "${clipname}" found`,
        );
      }
      const node = new AnimationNode(name, clip);
      node.playback_mode = playback_mode;
      clip.setEffectiveTimeScale(speed);
      am.register(node);
    };

    register("idle", "IDLE");
    register("run", "RUN");

    am.pair("idle", "run");
    am.pair("run", "idle", ANIMATION_TRANSITION_MODE.instant);

    this.animator.transite("idle");
  }

  step(dt) {
    this.animator.step(dt * 1e-3);

    this.velocity.copy(this.pos.sub(this._target.position));
    this.pos.copy(this._target.position);
    this.goal.y = this.pos.y;

    const rotation = this._target.rotation.y;
    const facing_direction = cache.vec3.v0
      .copy(Vec3Forward)
      .applyAxisAngle(Vec3Up, rotation);
    const goal_delta = cache.vec3.v1.copy(this.goal).sub(this.pos);
    const direction = cache.vec3.v2.copy(goal_delta).normalize();
    const direction_angle = Math.atan2(direction.x, direction.z);

    let rotate = angle_sub(rotation, direction_angle - Math.PI / 2);

    if (goal_delta.length() < 1e-1) {
      direction.multiplyScalar(0);
      rotate = 0;
    }

    this.apply_movement(dt, direction, rotate);

    if (this.velocity.length() > 1e-3) {
      this.animator.transite("run");
    } else {
      this.animator.transite("idle");
    }
  }

  apply_movement(dt, direction, rotate) {
    const df = dt / 30;
    const d = cache.vec3.v0.copy(direction);

    if (this.allow_move) {
      if (this.body) {
        d.multiplyScalar(1);
        d.y = 0;
        if (this.allow_move) {
          this.body.setLinearVelocity(d);
        }
      } else {
        d.multiplyScalar(df * 0.04);
        this._target.position.add(d);
      }
    }

    this._target.rotation.y += rotate * df * 0.1;
  }

  /**
   * @param {oimo.dynamics.rigidbody.RigidBody} body .
   */
  set_body(body) {
    this.body = body;
  }

  dispose() {
    this.body = null;
    this.gltf = null;
    this.animator = null;
    this._target = null;
  }
}

export default PawnDrawA;
