...
class VfxMeshWobble {
  ...
  step(dt) {
    ...
    acceleration.multiplyScalar(14).negate();
    ...
    dlerp_vec3(this.acceleration, acceleration, 0.5, sdt);
    ...
    this._target.traverse((o) => {
      ...
      const f0 = origin.y / this.max_y;

      const rot = this.cache.v3_0.copy(origin);
      rot.x = this.acceleration.z * f0;
      rot.z = -(this.acceleration.x - Math.abs(this.torque.y) * 2) * f0;
      //rot.applyAxisAngle(Vec3Up, o.rotation.z);

      pos.applyAxisAngle(Vec3Right, rot.x);
      pos.applyAxisAngle(Vec3Forward, rot.z * 0.3);

      dlerp_vec3(o.position, pos, 1, sdt);

      o.rotation.x = dlerp(o.rotation.x, rot.x, 0.3, sdt);
      o.rotation.z = dlerp(o.rotation.z, rot.z, 0.3, sdt);
    });
  }
  ...
  set_target(target) {
    ...
    target.traverse((o) => {
      ...
      this.max_y = Math.max(this.max_y, o.position.y);
    });
  }
